...
{
    $sql = <<<SQL
    SELECT pessoa.nome, funcionario.senha_hash as hash_senha, funcionario.codigo as codigo, medico.crm as isMedico
    FROM pessoa
    ...
    try {
        ...
        if (!$row)
            ...
            return array(password_verify($senha, $row['hash_senha']), $row['codigo'], $row['isMedico'], $row['nome']);
    } catch (Exception $e) {
